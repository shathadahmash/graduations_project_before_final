    ...
    <div className={containerClass}>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm mb-8 border border-slate-100">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
          <div className="lg:col-span-1">
            <label className="block text-xs font-black text-slate-600 uppercase mb-2 mr-1">بحث شامل</label>
            <div className="relative">
              <FiSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600" />
              <input
                type="text"
                placeholder="الاسم، البريد، الشركة..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pr-10 pl-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all text-sm"
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-black text-slate-600 uppercase mb-2 mr-1">الدور الوظيفي</label>
            <select
              value={filterRole}
          ...
          <div>
            <label className="block text-xs font-black text-slate-600 uppercase mb-2 mr-1">الجنس</label>
            <select
              value={filterGender}
          ...
          <div>
            <label className="block text-xs font-black text-slate-600 uppercase mb-2 mr-1">الحالة</label>
            <select
              value={filterStatus}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!editingUser && (
              <div className="space-y-1">
                ...
                <input type="text" placeholder="Username" value={newUser.username} onChange={e => setNewUser({ ...newUser, username: e.target.value })} className="w-full border border-slate-200 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white" />
              </div>
            )}
            <div className="space-y-1">
            ...
            {!editingUser && (
              <div className="space-y-1">
                ...
                <input type="password" placeholder="Password" value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} className="w-full border border-slate-200 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white" />
              </div>
            )}
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <select 
                ...
                className="w-full border border-slate-200 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white"
              >
            ...
            <div className="lg:col-span-3 flex gap-3 mt-4">
              ...
              <button 
                ...
                className="px-6 py-3 border border-slate-200 rounded-xl hover:bg-white transition-all font-bold text-slate-600"
              >
                إلغاء
              </button>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <table className={tableClass + ' text-right'}>
            <thead>
              <tr className="bg-white border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-wider">معلومات التواصل</th>
                <th className="px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-wider">الشركة / الدور</th>
                <th className="px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-wider">الحالة</th>
                <th className="px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-wider">تاريخ الانضمام</th>
                <th className="px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-wider">الإجراءات</th>
              </tr>
            </thead>
        ...
        {filteredUsers.length > visibleRows && (
          <div className="p-6 bg-white/50 border-t border-slate-100 flex flex-col items-center gap-4">
            <button
              onClick={() => setVisibleRows(prev => prev + 10)}
              className="flex items-center gap-2 px-8 py-3 bg-white border border-slate-200 rounded-xl text-sm font-black text-slate-700 hover:bg-white hover:border-slate-300 transition-all shadow-sm"
            >
        ...
        {visibleRows > 10 && (
          <div className="p-4 flex justify-center">
            <button
              ...
              className="text-xs text-slate-600 hover:text-slate-800 font-bold underline"
            >
              عرض أقل
            </button>
          </div>
