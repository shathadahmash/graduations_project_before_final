...
import { 
  FiCalendar, FiMapPin, FiBookOpen, FiUser, FiUsers, FiX, 
  FiSearch, FiSliders, FiInfo, FiEye, FiImage, FiArrowLeft, 
  FiUserCheck, FiLoader 
} from 'react-icons/fi';
import { projectService } from '../services/projectService';
import { userService } from '../services/userService';

// ... (جميع الـ interfaces كما هي)
...
interface ProjectFilterPageProps {
  ...
  showTitle?: boolean;
  embedded?: boolean; // للتضمين في صفحات أخرى
}

const ProjectFilterPage: React.FC<ProjectFilterPageProps> = ({
  initialFilters = {},
  onFilterChange,
  onProjectsLoaded,
  showTitle = true,
  ...
  const [filters, setFilters] = useState({
    ...
    supervisor: initialFilters.supervisor || '',
    co_supervisor: initialFilters.co_supervisor || '',
        ...
        groups.forEach((group: any) => {
          if (group.members && Array.isArray(group.members)) {
            group.members.forEach((member: any) => {
              ...
              if (user) {
                students.push({
                  name: user.name || `${user.first_name || ""} ${user.last_name || ""}`.trim() || user.username || "طالب",
                  id: user.id?.toString(),
                });
              }
            });
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      const processedData = data.map((p: any) => ({
        ...
        state: p.state?.name || p.state || 'غير محدد',
      ...
      if (onProjectsLoaded) onProjectsLoaded(processedData);
    } catch (err) {
  ...
  }, [filters, searchQuery, onProjectsLoaded]);
  ...
  // ... (مكونات الواجهة: FilterPanel, ActiveFilters, SearchBar, ProjectGrid, QuickViewModal)

  const FilterPanel = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-white p-4 rounded-xl shadow border border-[#31257D]/10">
      <select value={filters.university} onChange={e => setFilters(f => ({ ...f, university: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">الجامعة</option>
        {filterOptions.universities.map(u => <option key={`uni-${u.id}`} value={u.id}>{u.name}</option>)}
      </select>

      <select value={filters.college} onChange={e => setFilters(f => ({ ...f, college: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">الكلية</option>
        {filteredColleges.map(c => <option key={`col-${c.id}`} value={c.id}>{c.name}</option>)}
      </select>

      <select value={filters.department} onChange={e => setFilters(f => ({ ...f, department: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">القسم</option>
        {filteredDepartments.map(d => <option key={`dept-${d.id}`} value={d.id}>{d.name}</option>)}
      </select>

      <select value={filters.project_type} onChange={e => setFilters(f => ({ ...f, project_type: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">نوع المشروع</option>
        {filterOptions.project_types.map(t => <option key={`type-${t.value}`} value={t.value}>{t.label}</option>)}
      </select>

      <select value={filters.year} onChange={e => setFilters(f => ({ ...f, year: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">سنة المشروع</option>
        {filterOptions.years.map((y, i) => <option key={`year-${y}-${i}`} value={y}>{y}</option>)}
      </select>

      <select value={filters.field} onChange={e => setFilters(f => ({ ...f, field: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">المجال</option>
        {filterOptions.fields.map((f, i) => <option key={`field-${f}-${i}`} value={f}>{f}</option>)}
      </select>

      <select value={filters.tools} onChange={e => setFilters(f => ({ ...f, tools: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">الأدوات</option>
        {filterOptions.tools.map((t, i) => <option key={`tool-${t}-${i}`} value={t}>{t}</option>)}
      </select>

      <select value={filters.supervisor} onChange={e => setFilters(f => ({ ...f, supervisor: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">المشرف</option>
        {filterOptions.supervisors.map(s => <option key={`sup-${s.id}`} value={s.id}>{s.name}</option>)}
      </select>

      <select value={filters.co_supervisor} onChange={e => setFilters(f => ({ ...f, co_supervisor: e.target.value }))} className="p-2 border rounded-lg">
        <option value="">المشرف المساعد</option>
        {filterOptions.co_supervisors.map(s => <option key={`cosup-${s.id}`} value={s.id}>{s.name}</option>)}
      </select>
    </div>
  );
  ...
  const ActiveFilters = () => (
    <div className="flex flex-wrap gap-2 mb-4 bg-white p-3 rounded-lg border border-[#31257D]/10">
      ...
      {Object.entries(filters).map(([key, value]) => {
        ...
        let displayText = '';
        switch (key) {
          case 'university': displayText = filterOptions.universities.find(u => u.id === parseInt(value))?.name || value; break;
          case 'college': displayText = filterOptions.colleges.find(c => c.id === parseInt(value))?.name || value; break;
          case 'department': displayText = filterOptions.departments.find(d => d.id === parseInt(value))?.name || value; break;
          ...
          case 'project_type': displayText = `نوع: ${filterOptions.project_types.find(t => t.value === value)?.label || value}`; break;
        }
        return (
          <span key={key} className="bg-[#31257D]/10 text-[#31257D] text-xs px-3 py-1 rounded-full flex items-center gap-1">
            {displayText}
            <button onClick={() => setFilters(f => ({ ...f, [key]: '' }))} className="hover:text-red-600"><FiX size={12} /></button>
          </span>
        );
      })}
      <button onClick={clearAllFilters} className="text-red-600 hover:text-red-800 text-xs px-2 py-1 font-semibold">مسح الكل</button>
    </div>
  );
    ...
    <div className="flex flex-col md:flex-row gap-3 mb-4">
      ...
      <button
        className={`px-4 py-2 rounded-lg border flex items-center gap-2 transition-all ${
          showFilters ? 'bg-[#31257D] text-white border-[#31257D]' : 'bg-white text-[#31257D] border-[#31257D]/20 hover:bg-[#31257D]/5'
        }`}
      ...
      >
        ...
        {getActiveFiltersCount() > 0 && (
          <span className="bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{getActiveFiltersCount()}</span>
        )}
      </button>
    </div>
  );
  ...
  const ProjectGrid = () => (
    <>
      <div className="mb-4 text-sm text-[#4A5568] flex items-center justify-between">
        <span>تم العثور على <span className="font-bold text-[#31257D]">{projects.length}</span> مشروع</span>
        {(searchQuery || getActiveFiltersCount() > 0) && (
          <button onClick={clearAllFilters} className="text-red-600 hover:text-red-800 text-sm flex items-center gap-1">
            <FiX size={14} /> إعادة تعيين
          </button>
        )}
      </div>
          ...
          return (
            <div key={p.project_id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-[#31257D]/10 flex flex-col h-full group">
              <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[#31257D]/5 to-[#4937BF]/5">
                {p.logo ? (
                  <img src={getImageUrl(p.logo)} alt={p.title} className="w-full h-full object-cover transition-all duration-300" onError={(e) => { e.currentTarget.src = '/default-project-logo.png'; }} />
                ) : (
                ...
                {p.project_type !== 'غير محدد' && (
                  <div className="absolute top-3 right-3">
                    <span className={`${badge.bg} ${badge.color} px-3 py-1 rounded-full text-xs font-bold shadow-lg`}>{badge.label}</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-[#F8FAFC] p-2 rounded-lg">
                    <div className="flex items-center gap-1 text-[#4937BF] mb-1"><FiMapPin size={14} /><span className="text-xs">الجامعة</span></div>
                    <p className="font-medium text-[#31257D] text-sm line-clamp-1">{p.university_name}</p>
                  </div>
                  <div className="bg-[#F8FAFC] p-2 rounded-lg">
                    <div className="flex items-center gap-1 text-[#4937BF] mb-1"><FiBookOpen size={14} /><span className="text-xs">الكلية</span></div>
                    <p className="font-medium text-[#31257D] text-sm line-clamp-1">{p.college_name}</p>
                  </div>
                  <div className="bg-[#F8FAFC] p-2 rounded-lg">
                    <div className="flex items-center gap-1 text-[#4937BF] mb-1"><FiCalendar size={14} /><span className="text-xs">السنة</span></div>
                    <p className="font-medium text-[#31257D] text-sm">{extractYear(p.start_date) || 'غير محدد'}</p>
                  </div>
                  <div className="bg-[#F8FAFC] p-2 rounded-lg">
                    <div className="flex items-center gap-1 text-[#4937BF] mb-1"><FiUser size={14} /><span className="text-xs">المشرف</span></div>
                    <p className="font-medium text-[#31257D] text-sm line-clamp-1">{p.supervisor_name}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 mt-auto">
                  <button onClick={() => handleQuickView(p)} className="py-2.5 bg-[#31257D] text-white rounded-lg text-sm font-medium hover:bg-[#4937BF] transition-colors flex items-center justify-center gap-1">
  ...
  const QuickViewModal = () => (
    selectedProject && (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-all duration-300" onClick={() => { setSelectedProject(null); setSelectedProjectStudents([]); }}>
        <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-100 animate-fadeIn" onClick={e => e.stopPropagation()}>
          <div className="bg-gradient-to-l from-[#31257D] to-[#4937BF] p-5 flex justify-between items-center text-white">
            <div className="flex items-center gap-2"><FiInfo size={20} /><h2 className="text-xl font-bold">عرض سريع للمشروع</h2></div>
            <button onClick={() => { setSelectedProject(null); setSelectedProjectStudents([]); }} className="hover:bg-white/10 p-2 rounded-full transition-colors"><FiX size={22} /></button>
          </div>

          <div className="p-6 max-h-[70vh] overflow-y-auto">
            <div className="flex gap-4 mb-6">
              <div className="w-24 h-24 bg-[#F8FAFC] rounded-lg overflow-hidden border border-[#31257D]/10 flex-shrink-0">
                <img src={getImageUrl(selectedProject.logo)} alt={selectedProject.title} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.src = '/default-project-logo.png'; }} />
              </div>
              <div className="flex-1">
                ...
                {selectedProject.project_type !== 'غير محدد' && (
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${
                    ...
                    selectedProject.project_type === 'External' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'
                  }`}>{getProjectTypeLabel(selectedProject.project_type)}</span>
                )}
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-[#F8FAFC] p-3 rounded-lg"><p className="text-xs text-[#4A5568] mb-1">الجامعة</p><p className="font-medium text-[#31257D]">{selectedProject.university_name}</p></div>
              <div className="bg-[#F8FAFC] p-3 rounded-lg"><p className="text-xs text-[#4A5568] mb-1">الكلية</p><p className="font-medium text-[#31257D]">{selectedProject.college_name}</p></div>
              <div className="bg-[#F8FAFC] p-3 rounded-lg"><p className="text-xs text-[#4A5568] mb-1">المشرف</p><p className="font-medium text-[#31257D]">{selectedProject.supervisor_name}</p></div>
              <div className="bg-[#F8FAFC] p-3 rounded-lg"><p className="text-xs text-[#4A5568] mb-1">السنة</p><p className="font-medium text-[#31257D]">{extractYear(selectedProject.start_date)}</p></div>
            </div>

            <div className="bg-[#F8FAFC] p-4 rounded-lg mb-4">
              <h4 className="font-bold text-[#31257D] mb-2 flex items-center gap-2"><FiBookOpen className="text-[#4937BF]" />ملخص المشروع</h4>
              <p className="text-[#4A5568] text-sm line-clamp-3">{selectedProject.description || 'لا يوجد ملخص متاح'}</p>
            </div>

            <div className="bg-[#F8FAFC] p-4 rounded-lg mb-4">
              <h4 className="font-bold text-[#31257D] mb-3 flex items-center gap-2"><FiUsers className="text-[#4937BF]" />أعضاء المجموعة {loadingStudents && <FiLoader className="animate-spin mr-2" size={16} />}</h4>
              {loadingStudents ? (
                ...
                <div className="grid grid-cols-2 gap-2">
                  {selectedProjectStudents.map((student, index) => (
                    <div key={index} className="bg-white p-2 rounded-lg border border-[#31257D]/10 flex items-center gap-2">
                      <div className="w-8 h-8 bg-[#31257D]/10 rounded-full flex items-center justify-center"><FiUserCheck className="text-[#31257D]" size={16} /></div>
                      <div className="flex-1"><p className="font-medium text-[#31257D] text-sm">{student.name}</p>{student.id && <p className="text-xs text-gray-500">رقم: {student.id}</p>}</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <Link to={`/projects/${selectedProject.project_id}`} className="w-full py-3 bg-gradient-to-l from-[#31257D] to-[#4937BF] text-white rounded-lg font-bold hover:from-[#4937BF] hover:to-[#31257D] transition-all duration-300 flex items-center justify-center gap-2" onClick={() => { setSelectedProject(null); setSelectedProjectStudents([]); }}>
  ...
  return (
    <div className="w-full" dir="rtl">
      ...
      <SearchBar />
      {getActiveFiltersCount() > 0 && <ActiveFilters />}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-[#31257D]/5">
          ...
          <p className="text-[#4A5568] mb-4 max-w-md mx-auto">لم نتمكن من العثور على أي مشاريع تطابق معايير البحث الخاصة بك.</p>
          <button onClick={clearAllFilters} className="bg-[#31257D] text-white px-6 py-2 rounded-lg hover:bg-[#4937BF] transition-colors">إعادة تعيين الفلاتر</button>
        </div>
      ) : (
        <ProjectGrid />
      )}
