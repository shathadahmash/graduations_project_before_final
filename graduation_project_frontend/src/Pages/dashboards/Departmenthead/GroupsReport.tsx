...
const GroupsReport: React.FC = () => {
  ...
  const getDepartmentHeadDepartmentId = async (userId: number): Promise<number | null> => {
    try {
      ...
      console.error('[GroupsReport] Failed to fetch department head department:', err);
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Get department head's department using the same logic as GroupsTable
      const departmentHeadDepartmentId = user?.id ? await getDepartmentHeadDepartmentId(user.id) : null;

      const [groupsData, projectsData, affiliationsData] = await Promise.all([
        groupService.getGroups(),
        projectService.getProject(),
        userService.getAffiliations()
      ]);

      console.log('Groups Report - Department Head Department ID:', departmentHeadDepartmentId);

      // Filter groups by department head's department - groups belong to departments directly
      const filteredGroups = departmentHeadDepartmentId ? (groupsData || []).filter((g: any) => {
        // Get department ID from group (could be number or object with department_id field)
        let groupDepartmentId: number | null = null;
        if (g.department) {
          if (typeof g.department === 'number') {
            groupDepartmentId = g.department;
          } else if (typeof g.department === 'object') {
            groupDepartmentId = g.department.id || g.department.department_id;
          }
        }
        return groupDepartmentId && Number(groupDepartmentId) === Number(departmentHeadDepartmentId);
      }) : (groupsData || []);
