...
const SupervisorsReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Supervisors Report - Department Head Department ID:', deptHeadDepartmentId);
        console.log('Supervisors Report - All users:', allUsers.length);
        console.log('Supervisors Report - Affiliations:', affs.length);

        // Filter users who have the 'supervisor' role AND are from the same department
        // Use the same role filtering logic as SupervisorsTable.tsx (excluding co-supervisors)
        ...
        const supervisors = allUsers.filter(u => {
          const userDepartmentId = userDepartmentMap.get(u.id);
          const isSupervisor = u.roles.some(r => isSupervisorRole(r.type));
          const isSameDepartment = userDepartmentId === deptHeadDepartmentId;
          
          console.log(`User ${u.id} (${u.name}): roles=${u.roles?.map(r => r.type)}, isSupervisor=${isSupervisor}, department=${userDepartmentId}, isSameDepartment=${isSameDepartment}`);
          
          return isSupervisor && isSameDepartment && userDepartmentId !== undefined;
        });
        
        console.log('Supervisors Report - Filtered supervisors:', supervisors.length);
  ...
  // Statistics
  const stats = useMemo(() => {
    const total = filteredSupervisors.length;
    const active = filteredSupervisors.filter(u => u.is_active !== false).length;
    const male = filteredSupervisors.filter(u => u.gender === 'Male').length;
    const female = filteredSupervisors.filter(u => u.gender === 'Female').length;
    ...
    <div className="space-y-8 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">إدارة وتحليل بيانات أعضاء هيئة التدريس (المشرفين) في الكلية</p>
        </div>
        <div className="flex gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-xl text-sm font-bold hover:bg-amber-700 transition-all shadow-md">
            <FiDownload /> تصدير
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-amber-500 to-amber-600 p-6 rounded-2xl text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-amber-100 text-sm font-medium">إجمالي المشرفين</p>
