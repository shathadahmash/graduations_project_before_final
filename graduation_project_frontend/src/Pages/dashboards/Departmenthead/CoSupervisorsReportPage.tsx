  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('CoSupervisors Report - Department Head Department ID:', deptHeadDepartmentId);
        console.log('CoSupervisors Report - All users:', allUsers.length);
        console.log('CoSupervisors Report - Affiliations:', affs.length);

        // Filter users who have the 'co_supervisor' role AND are from the same department
        const coSupervisors = allUsers.filter(u => {
          ...
          console.log(`User ${u.id} (${u.name}): roles=${u.roles?.map(r => r.type)}, isCoSupervisor=${isCoSupervisor}, department=${userDepartmentId}, isSameDepartment=${isSameDepartment}`);
          
          return isCoSupervisor && isSameDepartment && userDepartmentId !== undefined;
        });
