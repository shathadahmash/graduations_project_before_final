  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Supervisors Report - Dean College ID:', deanCollegeId);
        console.log('Supervisors Report - All users:', allUsers.length);
        console.log('Supervisors Report - Affiliations:', affs.length);

        // Filter users who have the 'supervisor' role AND are from the same college
        ...
        const supervisors = allUsers.filter(u => {
          ...
          console.log(`User ${u.id} (${u.name}): roles=${u.roles?.map(r => r.type)}, isSupervisor=${isSupervisor}, college=${userCollegeId}, isSameCollege=${isSameCollege}`);
          
          return isSupervisor && isSameCollege && userCollegeId !== undefined;
        });
