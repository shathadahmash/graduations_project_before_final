...
const GroupsReport: React.FC = () => {
  ...
  const getDeanCollegeId = async (userId: number): Promise<number | null> => {
    try {
      ...
      console.error('[GroupsReport] Failed to fetch dean college:', err);
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Get dean's college using the same logic as GroupsTable
      ...
      console.log('Groups Report - Dean College ID:', deanCollegeId);

      // Filter groups by dean's college - groups belong to departments, which belong to colleges
      const filteredGroups = deanCollegeId ? (groupsData || []).filter((g: any) => {
